...
export const menubarMenuVariants = tv({
  base: 'flex h-menubar-bar items-center gap-menubar-bar-gap rounded-menubar-bar border border-border bg-background/95 p-menubar-bar-p shadow-menubar-bar',
})
...
export const menubarTriggerVariants = tv({
  base: [
    ...
    'text-foreground',
    'transition-colors duration-150 hover:bg-muted/70 focus:bg-muted data-[state=open]:bg-muted',
  ],
})

export const menubarContentVariants = tv({
  base: [
    'z-50 min-w-menubar-menu-content overflow-hidden rounded-menubar-menu-content p-menubar-menu-content shadow-menubar-menu-content ring-1 ring-foreground/5',
    ...overlaySurfaceBase,
  ],
})
